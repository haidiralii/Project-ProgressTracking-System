...
<div class="max-w-3xl mx-auto px-4 py-8">
    <div class="bg-white rounded-xl shadow-md border border-gray-200 p-8 animate-fade-in-up" style="animation-delay:0.1s;">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-lg font-semibold text-maroon flex items-center gap-2">
                ...
                <p class="text-sm text-gray-500 mt-1">Update job details for your project</p>
            </div>
            <a href="{{ route('projects.show', $job->project_id) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 text-maroon rounded-xl font-semibold hover:bg-maroon-lighter transition-all duration-200 shadow border border-maroon-light">
        ...
        @if(session('success'))
            <div class="mb-6 p-4 rounded-xl bg-green-100 text-green-800 border border-green-300 flex items-center animate-fade-in-up" style="animation-delay:0.2s;">
        ...
        <form action="{{ route('jobs.update', $job->id) }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-1 md:col-span-2 flex flex-col mb-4">
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            placeholder="Describe the job details...">{{ old('description', $job->description) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-gray-50 rounded-lg border border-gray-200 p-4 flex flex-wrap gap-4">
                    @foreach($operators as $operator)
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="operator_ids[]" value="{{ $operator->id }}" class="form-checkbox h-5 w-5 text-maroon rounded focus:ring-maroon-primary" @if(in_array($operator->id, $job->operators->pluck('id')->toArray())) checked @endif>
                            <span class="text-gray-800 font-medium">{{ $operator->name }}</span>
                            <span class="text-xs text-gray-500">({{ $operator->email }})</span>
                        </label>
                    @endforeach
                </div>
                <small class="text-gray-500">Click to select the operator to be assigned.</small>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="flex flex-col mb-4">
                        ...
                        <input type="date" id="requested_at" name="requested_at" required
                            class="rounded-md border border-gray-300 px-4 py-3 text-base focus:ring-2 focus:ring-maroon-primary focus:border-transparent bg-white transition-all duration-200"
                            value="{{ old('requested_at', $job->requested_at ? $job->requested_at->format('Y-m-d') : '') }}">
                    </div>
                    <div class="flex flex-col mb-4">
            ...
            <div>
                ...
                <div class="flex flex-col mb-4">
                    ...
                    <select name="status" class="rounded-md border border-gray-300 px-4 py-3 text-base focus:ring-2 focus:ring-maroon-primary focus:border-transparent bg-white transition-all duration-200" required>
